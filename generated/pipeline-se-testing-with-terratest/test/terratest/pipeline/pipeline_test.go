...
func TestPipeline_SE_UsEast1(t *testing.T) {
	// Run from this folder: test/terratest/pipeline
	cwd, err := os.Getwd()
	require.NoError(t, err)

	terraformDir := filepath.Clean(filepath.Join(cwd, "..", "..", "..", "infra", "terraform", "main"))

	varFile := filepath.Join(terraformDir, "aws-SE-99d097-databricks", "us-east-1", "terraform-pipeline.auto.tfvars")
