...
func TestAwsS3BucketMetastore_SE_UsEast1(t *testing.T) {
	// Run from this folder: test/terratest/aws_s3_bucket_metastore
	cwd, err := os.Getwd()
	require.NoError(t, err)

	terraformDir := filepath.Clean(filepath.Join(cwd, "..", "..", "..", "infra", "terraform", "main"))

	varFile := filepath.Join(terraformDir, "aws-SE-99d097-databricks", "us-east-1", "terraform-aws-s3-bucket-metastore.auto.tfvars")
