...
func TestServicePrincipal_SE_UsEast1(t *testing.T) {
	// Run from this folder: test/terratest/service_principal
	cwd, err := os.Getwd()
	require.NoError(t, err)

	terraformDir := filepath.Clean(filepath.Join(cwd, "..", "..", "..", "infra", "terraform", "main"))

	varFile := filepath.Join(terraformDir, "aws-SE-99d097-databricks", "us-east-1", "terraform-service-principal.auto.tfvars")
	...
	displayName := terraform.Output(t, opts, "display_name")
	aclPrincipalId := terraform.Output(t, opts, "acl_principal_id")

	require.NotEmpty(t, id)
	require.NotEmpty(t, applicationId)
